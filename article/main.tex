...
\textbf{Dynamic steering outperforms static steering.} Our orchestrator prototype demonstrates that switching domain-specific vectors at each plan step yields 4--7$\times$ more domain-relevant outputs than baseline, with zero degeneration. Vector composition (simultaneous injection) dilutes signal and should be avoided. The validated architecture---a steering library with sequential switching and adaptive coefficients---opens a path toward lightweight, training-free enhancement of SLM agents on benchmarks like SWE-bench Verified.
