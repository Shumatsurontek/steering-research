...
Our results suggest a promising architecture for multi-agent orchestration on SLMs: dynamically binding domain-specific steering vectors at each tool call step. The key evidence:

\begin{itemize}
    \item Steering vectors generalize across domains (calendar$\to$math reasoning) and model sizes (4B$\to$0.6B).
    \item The intervention is instantaneous (a single \texttt{register\_forward\_hook}) with negligible latency overhead.
    \item Zero-shot + steering outperforms both zero-shot alone and 5-shot alone on the instruct model (62\% vs.\ 46\% vs.\ 48\%).
\end{itemize}

The critical design requirement is \textbf{adaptive coefficient selection}. Our experiments demonstrate that the optimal $\alpha$ depends on at least three factors: (1)~the number of few-shot exemplars in the prompt, (2)~the model variant (instruct vs.\ base), and (3)~the model size (the stability window narrows on smaller models). A practical system would maintain a \textit{steering library}---a mapping from task domains to pre-extracted vectors with calibrated coefficients---and select the appropriate configuration based on the orchestrator's current plan step.
